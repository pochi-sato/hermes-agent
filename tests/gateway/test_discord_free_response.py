...
async def test_discord_free_response_channel_auto_threads(adapter, monkeypatch):
    """Free-response channels waive the @mention gate but still auto-thread.

    Only no_thread_channels suppresses auto-threading / inline parent replies.
    """
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
...
@pytest.mark.asyncio
async def test_discord_no_thread_channel_replies_inline(adapter, monkeypatch):
    """no_thread_channels still skip auto-threading and reply inline."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "false")
    ...
    adapter.handle_message.assert_awaited_once()
    event = adapter.handle_message.await_args.args[0]
    assert event.text == "inline reply please"
    assert event.source.chat_id == "789"
    assert event.source.chat_type == "group"


@pytest.mark.asyncio
async def test_discord_free_response_channel_in_no_thread_replies_inline(adapter, monkeypatch):
    """no_thread_channels wins when a channel is configured for both gates."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    monkeypatch.setenv("DISCORD_FREE_RESPONSE_CHANNELS", "789")
    monkeypatch.setenv("DISCORD_NO_THREAD_CHANNELS", "789")
    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)  # default true
    ...
    message = make_message(
        channel=FakeTextChannel(channel_id=789),
        content="mentionless but inline",
    )
    ...
    adapter.handle_message.assert_awaited_once()
    event = adapter.handle_message.await_args.args[0]
    assert event.text == "mentionless but inline"
    assert event.source.chat_id == "789"
    assert event.source.chat_type == "group"
